                    ...
                    <div className={styles.ingredientsContainer}>
                        {
                            orderInfo.ingredients.slice(0,6)
                                .map((ingredientId,index) => ingredientId && <div className={styles.ingredientImageContainer} data-count={`+${orderInfo.ingredients.slice(6).length}`} key={`${orderInfo._id}-${index}-${ingredientId}`}><img src={ingredientsData.getIngredientImage(ingredientId)} className={styles.ingredientImage} alt={"Картинка ингредиента"}/></div>)
                        }
                    </div>
